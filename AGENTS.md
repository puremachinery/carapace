...
- Do not mention generic engineering/ops basics that apply to all options.
- Assume migration, testing, and design effort are already planned and funded.
- Ignore "it depends" unless there is a single blocker that would actually flip the decision.
- Provide 2-3 concrete reasons only; no hedging.
- If uncertain, pick the most likely answer and state confidence in one sentence.
