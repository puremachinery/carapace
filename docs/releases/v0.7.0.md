...
## Breaking Changes
- Legacy persisted formats and migration-only readers are removed. PBKDF2/V1 backups, `enc:v1:` config/auth-profile secrets, V1 bare-array auth-profile stores, pre-versioned HMAC sidecars, and plaintext session artifacts under active session encryption now fail closed instead of migrating in place.
...
- If you rely on session integrity sidecars, expect pre-versioned `.hmac` sidecars to be rejected under `sessions.integrity.action = "reject"` and warned under `"warn"`; verify current sessions after first start so versioned sidecars are present.
