...
- Session integrity uses HMAC sidecars for metadata, history, and
  ...
  state is rejected. `manifest_integrity_failed` remains the public
  whole-session manifest authenticity error for manifest/HMAC root failures.
- Encrypted sessions require `.crypto-manifest` plus
  `CARAPACE_CONFIG_PASSWORD` for recovery.
- Matrix recovery-key rotation recovery now documents all
  `MatrixRecoveryKeyPromotionRefusalReason` wire values, fails closed when
  `pending_key_written` lacks a current key to compare against the recorded
  previous digest, and reports malformed typed markers as
  `matrix_recovery_key_rotation_marker_invalid` / `corrupt_typed_marker`
  without logging marker contents, paths, or key digests.
- `audit_blocking` writes synchronously to the state directory supplied by the
  caller even when a daemon audit writer is initialized elsewhere in-process.
