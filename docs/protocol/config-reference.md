...
- **`agents.defaults.model`**
  ...
  - *Control API note:* `/control/onboarding/status` exposes setup command hints for Control UI clients. `providers[].cliSetupCommand` is optional and is omitted when the top-level command would only be a `<model-id>` template. `providers[].cliSetupCommandNote` annotates it when the command has a caveat. `providers[].availableEntrypoints[].commandNote` annotates an individual entrypoint command. Clients must not run commands containing `<model-id>` until an operator has replaced the placeholder. See the [Control API onboarding status reference](http.md#get-controlonboardingstatus).
