...
| `carapace_matrix_pending_verifications` | gauge | none | Current daemon-side Matrix verification record count. Updated when verification records change; channel status metadata derives the same count directly from runtime state. |
